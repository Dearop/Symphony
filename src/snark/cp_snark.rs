...
pub fn encode_cp_instance_r1cs(
    folded_instance: &FoldedInstance,
    layout: &CpR1csLayout,
) -> Vec<u8> {
...
pub fn encode_cp_witness_r1cs(
    ...
    for ell in 0..ell_np {
        for i in 0..kappa {
            if ell < beta.len() && ell < commitments.len()
                && i < commitments[ell].value.elements.len()
            {
                let ntt_ctx = ntt.as_ref().expect("NTT context required for CP R1CS witness");
    ...
    let bb_red = |v: i128| -> i64 {
        ...
        if r > bb_p as i128 / 2 { r as i64 - bb_p as i64 } else { r as i64 }
    };
    ...
    for ell in 0..ell_np {
        ...
        for round_i in 0..had_nv {
            for pt in 0..4 {
                if round_i < sc.round_messages.len() && pt < sc.round_messages[round_i].evaluations.len() {
        ...
        let rc = |round: usize| -> (i64, i64) {
            if round < had_challenges.len() {
                (bb_red(had_challenges[round].c0 as i128), bb_red(had_challenges[round].c1 as i128))
            } else {
                (0, 0)
            }
        };
        let seed = |round: usize| -> (i64, i64) {
            if round < had_seed.len() {
                (bb_red(had_seed[round].c0 as i128), bb_red(had_seed[round].c1 as i128))
            } else {
                (0, 0)
            }
        };
        ...
        let newton_d3 = |round: usize, comp: usize| -> i64 {
            let e = |pt: usize| -> i64 { if comp == 0 { ev(round, pt).0 } else { ev(round, pt).1 } };
            bb_red(inv6 as i128 * bb_red(-(e(0) as i128) + 3 * e(1) as i128 - 3 * e(2) as i128 + e(3) as i128) as i128)
        };
        ...
        for round_i in 0..had_nv {
            ...
            _claim = (bb_red(c0 as i128 + e0_0 as i128), bb_red(c1 as i128 + e0_1 as i128));
        }
        ...
        for j in 1..d {
            ...
            buf.extend_from_slice(&c0.to_le_bytes());
            buf.extend_from_slice(&c1.to_le_bytes());
            combined = (bb_red(combined.0 as i128 + c0 as i128), bb_red(combined.1 as i128 + c1 as i128));
...
impl CpR1csLayout {
    pub fn new(ell_np: usize, kappa: usize, n_in: usize, m: usize) -> Self {
        let d = D;
        let had_num_vars = if m <= 1 { 0 } else { (usize::BITS - (m - 1).leading_zeros()) as usize };
        ...
        let had_data_per_instance =
            had_num_vars * 4 * 2   // evals
        ...
        Self {
            d, ell_np, kappa, n_in, had_num_vars,
            off_one, off_c_star, off_x_star, num_instance,
            off_beta, off_c, off_x_in, off_prod_c, off_prod_x,
            off_had, had_block_size, num_variables, had_aux_per_instance,
        }
    }
...
pub fn generate_cp_r1cs(
    ...
    for j in 0..d {
        ...
        for k in 0..d {
            ...
            ntt_coeff[j][k] = if val > p / 2 { val as i64 - p as i64 } else { val as i64 };
    ...
    for ell in 0..ell_np {
        for i in 0..kappa {
            for j in 0..d {
                ...
                for k in 0..d {
                    r1cs.c.insert(row, layout.prod_c(ell, i, k), ntt_coeff[j][k]);
                }
                row += 1;
            }
        }
    ...
    for ell in 0..ell_np {
        for s in 0..n_in {
            for j in 0..d {
                ...
                for k in 0..d {
                    r1cs.c.insert(row, layout.prod_x(ell, s, k), ntt_coeff[j][k]);
                }
                row += 1;
            }
        }
    ...
    fn ext_mul_lc(
        ...
        for &(v, c) in a_c0 { r1cs.a.insert(*row, v, c); }
        for &(v, c) in b_c0 { r1cs.b.insert(*row, v, c); }
        r1cs.c.insert(*row, p1, 1);
        *row += 1;

        // Row 1: Σ a_c1 * Σ b_c1 = p2
        for &(v, c) in a_c1 { r1cs.a.insert(*row, v, c); }
        for &(v, c) in b_c1 { r1cs.b.insert(*row, v, c); }
        ...
        for ell in 0..ell_np {
            ...
            for round_i in 0..had_nv {
                ...
                let d3 = |comp: usize| -> Vec<(usize, i64)> {
                    vec![(ev(0, comp), neg_inv6), (ev(1, comp), three_inv6),
                         (ev(2, comp), neg3_inv6), (ev(3, comp), inv6)]
                };
                let d2 = |comp: usize| -> Vec<(usize, i64)> {
                    vec![(ev(0, comp), inv2), (ev(1, comp), neg2_inv2), (ev(2, comp), inv2)]
                };
                ...
                ext_mul_lc(&mut r1cs, &mut row, layout.off_one, qnr,
                    &d3(0), &d3(1), &r_minus_2_c0, &r_c1, m1);
                ...
                ext_mul_lc(&mut r1cs, &mut row, layout.off_one, qnr,
                    &t2_c0, &t2_c1, &r_minus_1_c0, &r_c1, m2);
            ...
            for i in 1..had_nv {
                let m = alloc_kmul();
                ext_mul_lc(&mut r1cs, &mut row, layout.off_one, qnr,
                    &eq_c0, &eq_c1, &factor_vars[i].0, &factor_vars[i].1, m);
                eq_c0 = vec![(m.2, 1)];
                eq_c1 = vec![(m.3, 1)];
            }
            ...
            for j in 1..d {
                ...
                let m = alloc_kmul();
                ext_mul_lc(&mut r1cs, &mut row, layout.off_one, qnr,
                    &alpha_pow_c0, &alpha_pow_c1, &diff_c0, &diff_c1, m);
                ...
                if j + 1 < d {
                    let m2 = alloc_kmul();
                    ext_mul_lc(&mut r1cs, &mut row, layout.off_one, qnr,
                        &alpha_pow_c0, &alpha_pow_c1, &alpha_c0, &alpha_c1, m2);
            ...
            ext_mul_lc(&mut r1cs, &mut row, layout.off_one, qnr,
                &eq_c0, &eq_c1, &combined_c0, &combined_c1, final_m);
            ...
            for comp in 0..2 {
                r1cs.a.insert(row, layout.off_one, 1);
                r1cs.b.insert(row, if comp == 0 { final_m.2 } else { final_m.3 }, 1);
                for &(v, c) in if comp == 0 { &claim_c0 } else { &claim_c1 } {
                    r1cs.c.insert(row, v, c);
                }
                row += 1;
