...
The theorem does not assume the conclusion that the concrete proof is sound. It
assumes a separately checkable row-level implication: accepting concrete rows
must denote the same abstract WHIR-backed obligations used by \(\Rfold\). If the
rows merely carry digests or similarly named fields without enforcing the
corresponding verifier semantics, typed-batch faithfulness is false and the
theorem cannot be applied. What remains to audit is therefore concrete and
row-local: for each K6a/N8 row family, show that the public metadata, transcript
roots, challenge digests, openings, folding equations, and folded-output boundary
jointly imply the six obligations of
Lemma~\ref{lem:typed-row-faithfulness-sufficient}.
...
and~\ref{sec:formal-relations} to the current codebase. The implementation does
not expose a generic accumulator over arbitrary vectors of WHIR proof objects.
It exposes typed public batches and route-specific accumulator instances whose
rows are intended to represent the WHIR-backed obligations of the abstract
construction.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Routes and Data Model}
\label{sec:routes-and-data-model}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

There are three relevant implementation routes. The default
\texttt{prove\_public}/\texttt{verify\_public} route is the product WHIR+WHIR
public verifier. K6a is the explicit single-oracle full-workload accumulator
route. N8 is the explicit integrated multi-oracle alternative with
\(\mathsf{ACC.P}\), \(\mathsf{ACC.V}\), and \(\mathsf{ACC.D}\) entry points. K6a
and N8 are opt-in NonZK integrity routes; they do not silently replace the
default product verifier.

The implementation represents the abstract objects
\[
  ...
  \Pi_{\mathsf{cp}}
\]
by typed public structures rather than by raw tuples:
\[
\begin{array}{c|c}
...
Route-specific batches encode the public obligations corresponding to the
abstract WHIR statement/proof claims, plus the metadata needed to bind them to an
accumulator transition. This is why typed-batch faithfulness is the main
implementation obligation: the typed public rows must denote the same public
verification obligations as the abstract claim notation, not just carry digests
with compatible names.
...
In K6a, the accumulation workload is packaged into one public-canonical backend
table and one top-level WHIR backend proof for the typed CP relation. In N8, the
same public transition is organized into logical row families before being
proved by one integrated WHIR proof. These families include K6a semantic rows,
tuple-RLC semantic rows, and transition/binding rows.
...
\[
  ...
  \end{aligned}
\]
N8 is therefore evidence for the native multi-oracle direction because it
structures the backend statement without turning the route into a list of
independent WHIR verifications.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Encoded Folding Transition}
\label{sec:encoded-folding-transition}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The concrete routes encode the abstract transition
\[
  \Rfold\!\left(
    \Acc_{\mathsf{old}},
    \{\mathsf{Claim}_i\}_{i=1}^k,
  ...
  \right)=1.
\]
The typed rows must enforce shape compatibility, challenge derivation from the
declared public transcript, opening consistency, algebraic folding consistency,
and equality between the computed folded state and
\(\Acc_{\mathsf{new}}\). The CP relation is the public-verifier form of this
transition: it proves those checks while keeping folding witnesses, openings,
and auxiliary prover data outside the verifier boundary.

WHIR is then used as the backend proof system for the encoded CP relation. The
...
\[
  ...
  \mathsf{Enc}_{\WHIR}(\Rcp,I_{\mathsf{cp}},\mathsf{pp}_{\WHIR})
\]
and verifies \(\Pi_{\mathsf{cp}}\) against the recomputed statement. The backend
statement must bind the same public data as the CP instance: old and new
accumulators, typed batch, relation metadata, transcript roots, challenge
digests, and folded-output boundary. Omitting any of these values would make the
backend proof sound for a weaker statement than the advertised accumulator
transition.
...
The corresponding review matrix is:
\begin{enumerate}
...
These are the implementation counterparts of the binding assumptions used in the
soundness theorem.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Evaluation}
...
The construction and soundness argument above are conditional. They identify the
interface required for a sound WHIR-backed accumulator, but they do not by
themselves certify every concrete route as production-authoritative. The main
limitations are the following.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Relation and Transcript Faithfulness}
\label{sec:relation-encoding-limitations}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The central limitation is faithfulness of the encoded relation. The WHIR backend
is sound for the statement it actually proves; if that statement omits an
accumulator field, transcript digest, opening condition, or folded-output
boundary, then it may prove a weaker language than the accumulator API claims.
The K6a and N8 rows must therefore be checked against the ledger of
Section~\ref{sec:formal-relations}: claim representation, shape consistency,
challenge binding, opening consistency, folding consistency, and output
consistency.

This includes commitment compatibility. Lattice/Ajtai commitments, CP witness
commitments, and WHIR oracle commitments bind different objects. Soundness
requires explicit public digests and relation identifiers tying all of them to
the same transition. It also includes transcript binding: folding challenges,
WHIR backend challenges, and public verifier digests must be derived from the
same public statement material, and the verifier must reject if this cannot be
checked from public data.
...
The abstract notation uses a batch of WHIR proof-validity claims
\[
  \{\mathsf{Claim}_i\}_{i=1}^k,
\]
but the concrete single-oracle and multi-oracle routes consume typed
accumulation batches. They are implementations of the semantic interface, not
unrestricted generic accumulators for arbitrary WHIR proof objects. The current
K6a and N8 routes should therefore be viewed as soundness-guided NonZK
integrity prototypes, not as a complete zero-knowledge Symphony-style SNARK.
...
\begin{enumerate}
  \item The typed CP relation remains large, which motivates the native
        multi-oracle direction measured in the evaluation.
  \item The row-level faithfulness audit remains the main mathematical gap
        between the reduction theorem and concrete K6a/N8 soundness.
  \item The current routes are not zero-knowledge. Adding ZK would require
        masking the folding witness, openings, and auxiliary CP witness data,
        then proving the masked relation while binding masking commitments into
        the same transcript.
  \item The soundness theorem is one-step. Long-chain accumulation needs a
        \(\mathsf{ValidAcc}\) invariant and a global transcript and commitment
        analysis rather than only the conservative union bound of
        Section~\ref{sec:error-composition}.
  \item A production-authoritative route still needs a complete public-verifier
        audit: no witness-side replay, no debug-only fallback, no
        non-authoritative helper, and explicit binding of every public statement
        component.
\end{enumerate}
